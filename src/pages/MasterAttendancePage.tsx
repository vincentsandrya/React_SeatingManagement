...
import { 
  ...
  Edit3, CheckCircle2, Clock, ChevronLeft, ChevronRight, X, Search 
} from 'lucide-react';
...
export default function MasterAttendancePage() {
  ...
  // Modals
  const [selectedGuestQR, setSelectedGuestQR] = useState<GuestDWithRelation | null>(null);
  const [isBulkModalOpen, setIsBulkModalOpen] = useState(false);
  const [editingGuest, setEditingGuest] = useState<GuestDWithRelation | null>(null);
  ...
  useEffect(() => {
    const loadGuests = async () => {
      setIsLoading(true);
      try {
        const { data, count } = await guestService.getGuests(debouncedSearch, currentPage, pageSize);
  ...
  const handleExportCSV = async () => {
    setIsExporting(true);
    try {
      // Ambil data dalam jumlah besar sekaligus untuk export (mengikuti filter pencarian jika ada)
      ...
      // Header CSV
      const headers = ["Title", "Name", "Category", "Table", "Seat", "Dietary", "Status", "Check-in Time"];
  ...
  return (
    <div className="max-w-[1400px] mx-auto text-slate-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
        <div>
          <div className="text-xl font-bold mb-0.5 text-slate-900">Attendance Overview</div>
          <div className="text-xs text-slate-500">Live guest tracking and management.</div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden flex flex-col min-h-[460px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-200">
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider">Name</th>
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider">Category</th>
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider">Table</th>
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider">Seat</th>
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-wider text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-slate-500 text-xs">Loading data...</td>
                </tr>
              ) : guests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-slate-500 text-xs">No guests found.</td>
                </tr>
              ) : (
                guests.map((guest) => (
                  <tr key={guest.guest_d_id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-mono text-slate-700">{guest.table_number || '-'}</td>
                    <td className="px-4 py-3 text-xs font-mono text-slate-700">{guest.seat_number || '-'}</td>
        ...
        <div className="px-4 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-xs text-slate-500 mt-auto">
          <div>
            Showing {totalGuests === 0 ? 0 : (currentPage - 1) * pageSize + 1} - {Math.min(currentPage * pageSize, totalGuests)} of {totalGuests}
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center p-4 border-b border-slate-100 shrink-0">
              <div className="text-xs font-bold text-slate-900">Edit Guest Details</div>
            ...
            <form onSubmit={handleSaveEdit} className="flex flex-col overflow-hidden">
              <div className="p-5 space-y-4 overflow-y-auto">
                ...
                <div className="flex gap-3">
                  <div className="w-1/2">
                    <label className="block text-[11px] font-semibold text-slate-500 mb-1">Table</label>
                    <input 
                      type="text" 
                      name="table_number" 
                  ...
                  <div className="w-1/2">
                    <label className="block text-[11px] font-semibold text-slate-500 mb-1">Seat</label>
                    <input 
                      type="text" 
                      name="seat_number" 
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-4 border-b border-slate-100">
              ...
              <button onClick={() => setSelectedGuestQR(null)} className="text-slate-400 hover:text-slate-600">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 flex flex-col items-center justify-center bg-slate-50">
              <div className="bg-white p-3 rounded-lg shadow-sm border border-slate-200 mb-3">
                <QRCodeSVG value={selectedGuestQR.guest_h?.ticket_code || 'NO-TICKET'} size={160} level="H" />
              </div>
              <div className="text-sm font-bold text-slate-900">{selectedGuestQR.title} {selectedGuestQR.name}</div>
              <div className="text-xs font-mono text-slate-500 mt-1">{selectedGuestQR.guest_h?.ticket_code || 'NO-TICKET'}</div>
            </div>
            <div className="p-4 bg-white border-t border-slate-100">
              <button onClick={handlePrint} className="w-full bg-black text-white py-2 rounded text-xs font-semibold flex items-center justify-center gap-1.5 hover:bg-slate-800">
      ...
      {isBulkModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center p-4 border-b border-slate-100 shrink-0">
              <div>
                <div className="text-xs font-bold text-slate-900">Bulk Generate QR</div>
                <div className="text-[10px] text-slate-500">Generating QR for guests on this page.</div>
              </div>
            ...
            <div className="p-5 bg-slate-50 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 overflow-y-auto print-grid">
              {guests.map((guest) => (
                <div key={guest.guest_d_id} className="bg-white p-3 rounded border border-slate-200 flex flex-col items-center text-center shadow-sm">
                  <QRCodeSVG value={guest.guest_h?.ticket_code || 'NO-TICKET'} size={80} className="mb-2" />
                  <div className="text-[11px] font-bold text-slate-900 line-clamp-1">{guest.name}</div>
                  <div className="text-[9px] font-mono text-slate-500">{guest.guest_h?.ticket_code}</div>
                </div>
              ))}
            </div>

            <div className="p-4 bg-white border-t border-slate-100 flex justify-end gap-2.5 shrink-0">
              <button onClick={() => setIsBulkModalOpen(false)} className="px-4 py-2 text-xs font-semibold border border-slate-300 rounded hover:bg-slate-50">
                Cancel
              </button>
              <button onClick={handlePrint} className="px-5 py-2 bg-black text-white rounded text-xs font-semibold flex items-center gap-1.5 hover:bg-slate-800">
